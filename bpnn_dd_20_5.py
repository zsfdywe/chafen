...
for i in range(len(train)-25):
    train_x.append(np.array(train.values[i:i+20, :].flatten()))
    train_y.append(np.array(train.values[i+20:i+25, :].flatten()))
...
# print(test_x)
test_x = train.values[-20:, :].reshape((1, -1))
print(test_x)


xs = tf.placeholder(tf.float32, [None, 180])
ys = tf.placeholder(tf.float32, [None, 45])

Weights_1 = tf.Variable(tf.random_normal([180, 120])*0.001)
biases_1 = tf.Variable(tf.zeros([1, 120]) + 0.001)
Wx_plus_b_1 = tf.matmul(xs, Weights_1) + biases_1

layer1_out = tf.nn.relu(Wx_plus_b_1)

Weights_2 = tf.Variable(tf.random_normal([120, 80])*0.001)
biases_2 = tf.Variable(tf.zeros([1, 80]) + 0.001)
Wx_plus_b_2 = tf.matmul(layer1_out, Weights_2) + biases_2

layer2_out = tf.nn.relu(Wx_plus_b_2)

Weights_3 = tf.Variable(tf.random_normal([80, 45])*0.001)
biases_3 = tf.Variable(tf.zeros([1, 45]) + 0.001)

prediction = tf.matmul(layer2_out, Weights_3) + biases_3
loss = tf.reduce_mean(tf.reduce_sum(tf.square(ys - prediction), reduction_indices=[1]))
train_step = tf.train.AdamOptimizer(learning_rate=0.01, beta1=0.9, beta2=0.999).minimize(loss)
...
for i in range(40):
    ...
    print(y_hat)
    y_hat = np.array(y_hat).reshape((5, 9))
    output.append(y_hat)
    y_hat = np.array(y_hat).flatten()
    print(y_hat)
    test_x = list(test_x[:, 45:].flatten())
    print(test_x)
    # print(test_x)
    test_x.extend(y_hat)
    print(test_x)
