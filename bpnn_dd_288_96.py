...
train_x = []
train_y = []
for i in range(len(train)//96-4):
    train_x.append(np.array(train.values[i*96:(i+3)*96, :].flatten()))
    train_y.append(np.array(train.values[(i+3)*96:(i+4)*96, :].flatten()))
...
# print(test_x)
test_x = train.values[-96*3:, :].reshape((1, -1))
print(test_x.shape)


xs = tf.placeholder(tf.float32, [None, 96*3*9])
ys = tf.placeholder(tf.float32, [None, 96*9])

Weights_1 = tf.Variable(tf.random_normal([96*3*9, 320])*0.001)
biases_1 = tf.Variable(tf.zeros([1, 320]) + 0.001)
Wx_plus_b_1 = tf.matmul(xs, Weights_1) + biases_1

layer1_out = tf.nn.relu(Wx_plus_b_1)

Weights_2 = tf.Variable(tf.random_normal([320, 180])*0.001)
biases_2 = tf.Variable(tf.zeros([1, 180]) + 0.001)
Wx_plus_b_2 = tf.matmul(layer1_out, Weights_2) + biases_2

layer2_out = tf.nn.relu(Wx_plus_b_2)

Weights_3 = tf.Variable(tf.random_normal([180, 96*9])*0.001)
biases_3 = tf.Variable(tf.zeros([1, 96*9]) + 0.001)
...
for i in range(2):
    ...
    print(y_hat)
    y_hat = np.array(y_hat).reshape((96, 9))
    output.append(y_hat)
    y_hat = np.array(y_hat).flatten()
    print(y_hat)
    test_x = list(test_x[:, 96*9:].flatten())
    print(test_x)
    test_x.extend(y_hat)
    print(test_x)
...
output = np.array(output).reshape((-1, 9))
print(output)
plt.plot(output)
plt.show()
output = pd.DataFrame(output)
output.to_excel('y_hat_288_96.xlsx')


# outp =[ ]
# for j in range(output[0]):
#     outp.append(output[:, j] * scale[1][j] + scale[0][j])
#
# # for j in range(9):
# #     y_hat[j] = y_hat[j]*scale[1][j]+scale[0][j]
# #
# #
# output = pd.DataFrame(outp)
# output.to_excel('y_hat.xlsx')